...
public abstract class Query implements Cloneable {
	...
	public int update(Object obj, String[] fieldNames) {
		...
		params.add(ReflectUtils.invokeGet(obj, priKey.getName())); // 主键的值
			...
			public Object doExecute(Connection conn, PreparedStatement ps, ResultSet rs) {
				...
				try {
					...
					while (rs.next()) {
						...
						// 多列 select username ,pwd,age from user where id>? and
						// age>18
						for (int i = 0; i < metaData.getColumnCount(); i++) {
							String columnName = metaData.getColumnLabel(i + 1); // username
							Object columnValue = rs.getObject(i + 1);

							// 调用rowObj对象的setUsername(String
							// uname)方法，将columnValue的值设置进去
